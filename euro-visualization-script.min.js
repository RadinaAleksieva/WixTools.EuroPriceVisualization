...
function appendBGN(el, value, color, fontSize) {
  var existing = el.querySelector(".bgn-price");
  if (existing) {
    existing.textContent = "/ " + value + " лв.";
    return;
  }
...
function updateInnerTextBGN(text, bgnValue) {
  if ((text || "").indexOf("лв") !== -1) {
    return text.replace(/\/\s*[\d.,]+\s*лв\.?/g, "/ " + bgnValue + " лв.");
  }
  return (text || "") + "/ " + bgnValue + " лв.";
}
...
function convertWithInnerText(selectors, roundResult) {
  selectors.forEach(function (selector) {
    forEachNode(selector, function (el) {
      ...
      var source = normalizeSourceText(rawText);
      if (el.getAttribute("data-bgn-source") === source) return;
      var converted = convertPriceText(rawText, roundResult);
      if (converted) {
        el.innerText = updateInnerTextBGN(rawText, converted);
        el.setAttribute("data-bgn-source", source);
      }
    });
  });
}
...
function convertWithAppending(selectors) {
  selectors.forEach(function (selector) {
    forEachNode(selector, function (el) {
      ...
      var rawText = el.innerText || "";
      var source = normalizeSourceText(rawText);
      if (el.getAttribute("data-bgn-source") === source && el.querySelector(".bgn-price")) return;
      if (!el.querySelector(".bgn-price") && el.innerHTML.indexOf("лв") !== -1) return;
      var converted = convertPriceText(rawText);
      if (converted) {
...
function convertProductPagePrice() {
  forEachNode('div[data-testid="richTextElement"]', function (el) {
    ...
    if (!target || NotIncludesEuro(target)) return;
    var rawText = target.innerText || "";
    var source = normalizeSourceText(rawText);
    if (target.getAttribute("data-bgn-source") === source && target.querySelector(".bgn-price")) return;
    if (!target.querySelector(".bgn-price") && target.innerHTML.indexOf("лв") !== -1) return;
    var converted = convertPriceText(rawText);
    if (converted) {
      appendBGN(target, converted);
      target.setAttribute("data-bgn-source", source);
    }
  });
}
...
function convertSideCartTotalFallback() {
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    var steps = 0;
    while (sib && steps < 4) {
      var rawText = sib.textContent || "";
      if (rawText.indexOf("€") !== -1) {
        var source = normalizeSourceText(rawText);
        if (!(sib.getAttribute("data-bgn-source") === source && sib.querySelector(".bgn-price"))) {
          var converted = convertPriceText(rawText);
          if (converted) {
            appendBGN(sib, converted, sib.style.color, sib.style.fontSize);
            sib.setAttribute("data-bgn-source", source);
          }
        }
        break;
      }
...
function convertThankYouPrices() {
  convertWithInnerText([
    '[data-hook="ProductLineItemDataHook.price"]',
