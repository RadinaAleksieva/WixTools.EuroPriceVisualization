const RATE_BGN = 0.511292;
  ...
  function convertPriceText(t, e) {
    ...
    if (o.length === 1) {
      ...
      o[0][0] && o[0][0].match(DIGIT_REGEX) && o[1][0] && o[1]
  [0].match(DIGIT_REGEX)
        ? (n = o[0][0] + o[1][0])
    ...
    return isNaN(r) ? void 0 : e ? (r / RATE_BGN).toFixed(0) : (r /
  RATE_BGN).toFixed(2).replace(".", ",");
  }
  ...
  function NotIncludesEuro(t) {
    return !t.innerText.includes("€") && !t.innerText.startsWith("EUR") && !
  t.innerText.endsWith("EUR");
  }

  function convertCategoryPrices() {
    convertWithAppending(['[data-hook="product-item-price-to-pay"]', '[data-
  hook="formatted-primary-price"]', '[data-hook="challenge-pricing"]']);
  }
  ...
  function convertThankYouPrices() {
    convertWithInnerText(['[data-hook="ProductLineItemDataHook.totalPrice"]',
  '[data-hook="subtotal-row-value"]', '[data-hook="challenge-pricing"]']);
  }

  function convertFilter() {
    convertWithInnerText(['[data-hook="filter-type-PRICE"] span'], true);
    document.querySelectorAll('[data-hook="filter-type-PRICE"] span').forEach((t
  => { t.style.fontSize = "17px"; }));
  }

  function convertShipping() {
    convertWithAppending(['[data-hook="dropdown-option"]']);
  }

  function convertMembers() {
    convertWithInnerText(['[data-hook="subtotal"]', '[data-hook="shipping"]',
  '[data-hook="tax-0"]', '[data-hook="total"]', '[data-hook="product-total"]',
  '[data-hook="product-price"]', '[data-hook="value"]']);
  }
  ...
  window.addEventListener("load", (() => {
    function t(t) {
      t && (typeof t.remove === "function" ? t.remove() : t.parentNode &&
  t.parentNode.removeChild(t));
    }
    function e() {
      const e = document.querySelector('[data-hook="place-order-button"]');
      e && (e.dataset.listenerAttached || (e.dataset.listenerAttached = "true",
  e.addEventListener("click", (function () {
        window.matchMedia("(max-width: 768px)").matches && [".bgn-
  price"].forEach((function (e) {
          var o = document.querySelectorAll(e);
          o.forEach ? o.forEach((function (e) { t(e); })) :
  Array.prototype.forEach.call(o, (function (e) { t(e); }));
        }));
      }))));
    }
    setTimeout((() => { setInterval((() => { convertAllPrices(); }), 200); }),
  150);
    setTimeout((() => { setInterval((() => { e(); }), 1200); }), 1500);
    new MutationObserver(e).observe(document.body, { childList: !0, subtree: !
  0 });
    e();
  }));
